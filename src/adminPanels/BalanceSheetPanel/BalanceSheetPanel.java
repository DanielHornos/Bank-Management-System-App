package adminPanels.BalanceSheetPanel;
...
public class BalanceSheetPanel {
	...
	protected void actionForShowButton(ActionEvent e) {
		...
		// Check if bank account is valid or not
		 try {
			...
			String st = " SELECT * FROM Bank.dbo.BankAccounts WHERE AccountNr = '"
					+ id.getText()
					+ "'";
			...
			if (!rs.isBeforeFirst() ) { 
				...
				alert.setContentText("The account number introduced doesn't exist."+System.lineSeparator()+"Please introduce another Account Number.");
				alert.showAndWait();				
			    return;
			}
		}catch(Exception exc) {
