package userPanels.userTransferPanel;
...
public class UserTransferPanel {
	...
	protected void actionForConfirmTransferButton(ActionEvent e) throws IOException {
		 ...
		 try {
			...
			if (!rs.isBeforeFirst() ) { 
				...
				alert.setContentText("The account number 1 doesn't exist."+System.lineSeparator()+"Please press SHOW ACCOUNTS DETAILS button.");
				alert.showAndWait();
			    return;
			}
		}catch(Exception exc) {
	...
	protected void actionForShowDetailsButton(ActionEvent e) throws IOException {
	    ...
	    account1.setText(conn.getAccountNr());
		...
		try {			
			...
			Statement statement2 = getConn().getStat();
			String sql2 = "SELECT Name FROM Bank.dbo.BankAccounts WHERE AccountNr = '" + account2.getText() + "'";
			ResultSet resultSetBalance2 = statement2.executeQuery(sql2);
